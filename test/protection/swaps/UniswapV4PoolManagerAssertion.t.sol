...
import {UniswapV4PoolManagerHelpers} from "../../../src/protection/swaps/examples/UniswapV4PoolManagerHelpers.sol";
...
contract UniswapV4PoolManagerAssertionHarness is UniswapV4PoolManagerHelpers {
    ...
    function isNativeCurrency(address currency) external pure returns (bool) {
        return _isNativeCurrency(currency);
    }
}
...
contract UniswapV4PoolManagerAssertionTest is Test {
    ...
    function setUp() public {
        ...
        IUniswapV4PoolManagerLike.PoolKey memory key = IUniswapV4PoolManagerLike.PoolKey({
            currency0: c0, currency1: c1, fee: 3000, tickSpacing: 60, hooks: address(0)
        });
    ...
    function testUniswapV4SelectorsMatchExpectedSignatures() external pure {
        ...
        assertEq(
            ...
            bytes4(
                keccak256("modifyLiquidity((address,address,uint24,int24,address),(int24,int24,int256,bytes32),bytes)")
            ),
            "modifyLiquidity"
        );
        assertEq(
    ...
    function testRejectsMisorderedCurrencies() external {
        IUniswapV4PoolManagerLike.PoolKey memory key = IUniswapV4PoolManagerLike.PoolKey({
            currency0: address(0xFFFF), currency1: address(0x1111), fee: 3000, tickSpacing: 60, hooks: address(0)
        });
    ...
    function testRejectsZeroManager() external {
        IUniswapV4PoolManagerLike.PoolKey memory key = IUniswapV4PoolManagerLike.PoolKey({
            currency0: address(0x1111), currency1: address(0xFFFF), fee: 3000, tickSpacing: 60, hooks: address(0)
        });
