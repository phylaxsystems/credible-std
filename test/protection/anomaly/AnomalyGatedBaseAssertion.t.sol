...
// 512-bit `mulDivDown` overflow regression, codeless-token fail-open, and a fuzz property. The
// verdict oracle, given the anomaly trigger has fired: block iff
// `net * 10_000 / preBalance >= fracBps`. As in the other anomaly suites, a tx-end trigger stands
// in for the anomaly trigger.
...
contract TestAnomalyDrainRatio is CredibleTest, Test {
    ...
    function _register(uint256 fracBps) internal {
        cl.assertion({
            adopter: address(vault),
            createData: abi.encodePacked(
                type(CompositeTxEndHarness).creationCode, abi.encode(_config(address(token), fracBps))
            ),
