...
abstract contract UniswapV4PoolManagerHelpers is Assertion {
    ...
    function _snapshotAt(PhEvm.ForkId memory fork) internal view returns (PoolSnapshot memory snapshot) {
        ...
        snapshot.managerBalance0 = _currencyBalanceAt(CURRENCY0, MANAGER, fork);
        snapshot.managerBalance1 = _currencyBalanceAt(CURRENCY1, MANAGER, fork);
    }
    ...
    {
        return abi.decode(
            _args(input), (IUniswapV4PoolManagerLike.PoolKey, IUniswapV4PoolManagerLike.SwapParams, bytes)
        );
    }
    ...
    {
        return abi.decode(
            _args(input), (IUniswapV4PoolManagerLike.PoolKey, IUniswapV4PoolManagerLike.ModifyLiquidityParams, bytes)
        );
    }
