...
/// - manager-level checks protect the singleton accounting that backs protocol-fee liabilities for
///   the currencies used by the configured pool;
/// - PoolKey filtering prevents activity in unrelated pools from producing false positives while
///   still letting one assertion deployment monitor a specific pool inside the singleton.
contract UniswapV4PoolManagerAssertion is UniswapV4PoolManagerHelpers {
    ...
    ///      pre-call and post-call snapshots for the matched manager operation. The assertion
    ///      functions that receive a PoolKey return early when the call targets a different pool.
    ...
    /// @notice Registers the tx-end protocol-fee custody check.
    /// @dev Fee custody is global per currency in v4, and swap/donate calls may accrue fees before
    ///      the surrounding unlock flow settles token custody. Checking at tx end avoids tripping on
    ///      ordinary mid-flow accounting while still requiring final protocol-fee liabilities to be
    ///      backed by PoolManager custody for ERC-20 currencies.
    ...
    ///      explicit limit that bounds user execution. This protects the user-facing swap guarantee
    ///      rather than duplicating v4's calldata validation.
    ...
    ///      Slot0 (price + tick) must also be unchanged because modifyLiquidity is not allowed to
    ///      move the pool. This catches accounting corruption that may not be visible until a later
    ///      swap prices against the wrong active liquidity.
    ...
    /// @dev Swaps and donates can accrue protocol fees before the surrounding unlock flow settles
    ///      token custody, while collectProtocolFees withdraws them. The PoolManager singleton holds
    ///      tokens for every pool, but `protocolFeesAccrued` is summed per currency across all pools,
    ///      so the invariant `manager.balanceOf(currency) >= protocolFeesAccrued(currency)` must hold
    ///      globally at transaction end for ERC-20 currencies used by the configured pool.
    function assertProtocolFeesCoveredByCustody() external view {
        _requireConfiguredManagerIsAdopter();
        PoolSnapshot memory post = _snapshotAt(_postTx());

        require(post.managerBalance0 >= post.protocolFeesAccrued0, "UniswapV4Pool: currency0 protocol fees uncovered");
        require(post.managerBalance1 >= post.protocolFeesAccrued1, "UniswapV4Pool: currency1 protocol fees uncovered");
    }
    ...
    ///      assert the per-pool no-mutation invariant. This protects against singleton side effects
    ///      where a fee withdrawal for one currency unexpectedly contaminates the watched pool.
    function assertCollectProtocolPreservesPoolState() external view {
        ...
        require(post.slot0.protocolFee == pre.slot0.protocolFee, "UniswapV4Pool: collectProtocol changed protocolFee");
        ...
        require(
            post.feeGrowthGlobal0X128 == pre.feeGrowthGlobal0X128, "UniswapV4Pool: collectProtocol changed feeGrowth0"
        );
        require(
            post.feeGrowthGlobal1X128 == pre.feeGrowthGlobal1X128, "UniswapV4Pool: collectProtocol changed feeGrowth1"
        );
    ...
    function _requireCollectProtocolCustodyMatches(
        ...
        if (!isNativeCurrency) {
            ...
            require(accruedDelta == balanceDelta, _msg("UniswapV4Pool: collectProtocol custody mismatch", currencyTag));
        }

        if (requestedAmount != 0) {
            require(
                accruedDelta == requestedAmount, _msg("UniswapV4Pool: collectProtocol amount mismatch", currencyTag)
            );
        } else {
